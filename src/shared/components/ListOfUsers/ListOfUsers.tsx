...
export const ListOfUsers = observer(
  ({
    ...
    const renderedUsers = users ?? store.users;

    const canEditTargetUser = (user: User): boolean => canEditUser?.(user) ?? canManageUsers;
    const canDeleteTargetUser = (user: User): boolean =>
      canDeleteUser?.(user) ?? (canManageUsers && !isSelfUser(user));

    const handleDelete = (id: string) => {
      const targetUser = renderedUsers.find((user) => user.id === id);

      if (!targetUser || !canDeleteTargetUser(targetUser)) {
        return;
      }
    ...
    const handleConfirmDelete = () => {
      ...
      const targetUser = renderedUsers.find((user) => user.id === userIdForDelete);
            ...
            <div key={user.id} className={styles.row}>
              ...
              <div>
                {(() => {
                  ...
                  return <span className={`${styles.status} ${statusClass}`}>{statusLabel}</span>;
                })()}
              </div>
              {canEditTargetUser(user) || canManageUsers || canDeleteTargetUser(user) ? (
                <div className={styles.actions}>
                  ...
                  {canManageUsers && (
                    <button className={styles.button} aria-label="Поделиться пользователем">
                      <svg
                        width="20"
                        height="22"
                        viewBox="0 0 20 22"
