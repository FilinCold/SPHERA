...
export const ListOfUsers = observer(({ store: externalStore }: ListOfUsersProps) => {
  ...
  const handleShare = () => {
    ...
    /*
    // Пример реального запроса для шаринга пользователя
    await fetch(`/api/users/${id}/share`, {
      method: "POST",
  ...
  const handleEdit = (id: number) => {
    console.log("Редактировать пользователя:", id); //здесь должна быть логика открытия поп-апа (посоветоваться с Глебом)
    /*
    // Пример реального запроса на получение данных пользователя для редактирования
